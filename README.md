...
Model changes are rejected while a prompt, another switch, or unconsumed input is pending. Prompts wait for an existing switch; closing the session during it prevents replacement publication. After a failed switch, persisted sessions can be recovered with `session/resume`; an untouched session without a log requires `session/new`.
