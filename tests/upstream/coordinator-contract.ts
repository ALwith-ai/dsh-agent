// Vendored from deepseek-harness tag dsh-v0.1.2-rc.1 (packages/session/session-persistence/tests/coordinator-contract.ts); local patch: bun:test imports, package import path, waitFor shim, one resolves.not.toThrow() awaited directly, ts-nocheck.
// @ts-nocheck
...
import { meta, oneTurnLog, appendLog } from './persistence-contract.ts'
 ...
 */
/** bun:test has no vi.waitFor: poll the assertion until it passes or 2s elapse. */
  ...
  describe(`PersistenceCoordinator orchestration: ${name}`, () => {
    ...
    it('HMR: reloading the backend adopts a still-live, already-materialized session', async () => {
      ...
      try {
        ...
        await ctx.sessions.flush(session) // local patch: bun's resolves.not.toThrow() misreports here
